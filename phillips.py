...
def build_figure(data):
    ...
    fig.update_layout(template='plotly_white',height=900,font=dict(family='Arial',color='#172b4d',size=14),title=frames[0].layout.title,margin=dict(l=90,r=45,t=150,b=250),xaxis=dict(title='Tasa de desocupación (%) · trimestre móvil',range=limits(pd.concat([d.desocupacion,pd.Series(NAIRU_RANGO)])),ticksuffix='%'),yaxis=dict(title='Variación anual del IPC (%)',range=limits(pd.concat([d.ipc_anual,pd.Series([META_INFLACION])])),ticksuffix='%'),showlegend=False,
        annotations=[dict(x=0,y=1.10,xref='paper',yref='paper',showarrow=False,xanchor='left',text='Área ∝ |IR real anual| · verde: aumento · naranja: caída'),dict(x=0,y=-.43,yanchor='top',xref='paper',yref='paper',showarrow=False,xanchor='left',text='Fuente: INE · ENE asignada al mes central · sin ajuste estacional<br>NAIRU: rango BCCh 2024-T3 (publicación dic. 2024); línea = punto medio, no estimación oficial 2026.')],
    ...
    # Referencias en layout: persisten al reproducir, pausar y recorrer los frames.
    fig.add_vrect(x0=NAIRU_RANGO[0],x1=NAIRU_RANGO[1],fillcolor='rgba(124,58,237,0.08)',line_width=0,layer='below')
    fig.add_vline(x=NAIRU_REFERENCIA,line_color='#7c3aed',line_dash='dashdot',line_width=2)
    fig.add_hline(y=META_INFLACION,line_color='#2563eb',line_dash='dash',line_width=2)
    fig.add_annotation(x=NAIRU_REFERENCIA,y=.985,xref='x',yref='paper',text='NAIRU ref. 8,25%*',showarrow=False,xanchor='left',yanchor='top',xshift=8,font=dict(color='#6d28d9',size=13),bgcolor='rgba(255,255,255,.85)')
    fig.add_annotation(x=1,y=META_INFLACION,xref='paper',yref='y',text='Meta de inflación: 3%',showarrow=False,xanchor='right',yanchor='bottom',yshift=5,font=dict(color='#1d4ed8',size=13),bgcolor='rgba(255,255,255,.85)')
...
def static_chart(d, out=ROOT/'resultados'/'phillips_estatico.png'):
    ...
    ax.axvspan(*NAIRU_RANGO,color='#7c3aed',alpha=.07,zorder=0)
    ax.axvline(NAIRU_REFERENCIA,color='#7c3aed',ls='-.',lw=1.5,label='NAIRU ref. 8,25%*',zorder=1)
    ax.axhline(META_INFLACION,color='#2563eb',ls='--',lw=1.5,label='Meta de inflación 3%',zorder=1)
    ...
    fig.text(.01,-.035,'Fuente: INE y BCCh. Área proporcional a |IR real anual|.\n* Punto medio del rango 8,0–8,5% para 2024-T3, publicado en dic. 2024; no es una estimación oficial para 2026.',fontsize=9)
